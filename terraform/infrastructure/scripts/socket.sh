...
cd rewind/backend/sockets
source ~/.nvm/nvm.sh
nvm use 18 >> /dev/null
sudo cp nginx.conf /etc/nginx/nginx.conf >> /dev/null
sudo systemctl restart nginx && sudo systemctl enable nginx >> /dev/null
aws secretsmanager get-secret-value --secret-id rewind/backend/sockets --query "SecretString" --output text | jq -r "to_entries|map(\"\(.key)=\(.value|tostring)\")|.[]" > .env
npm install >> /dev/null
npm install -g typescript pm2 >> /dev/null
tsc >> /dev/null
touch logs.txt
pm2 start dist/app.js --name rewind-sockets --log logs.txt --time >> /dev/null
